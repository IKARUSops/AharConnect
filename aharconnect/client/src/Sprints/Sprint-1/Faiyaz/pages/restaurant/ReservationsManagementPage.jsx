...
const mockEnhancedReservations = [
  {
    id: 'res1',
    restaurantId: '1',
    customerId: 'cust1',
...
const ReservationsManagementPage = (props) => {
  const [reservations, setReservations] = useState(mockEnhancedReservations);
  const [eventPackages, setEventPackages] = useState(mockEventPackages);
  const [eventBookings, setEventBookings] = useState(mockEventBookings);
  ...
  // Handle event booking status change
  const handleEventBookingStatusChange = (bookingId, newStatus) => {
    ...
    // Show confirmation notification
    if (newStatus === 'confirmed') {
      toast.success(`Event booking confirmed! Notification sent to customer.`);
    } else if (newStatus === 'canceled') {
      toast.info(`Event booking canceled. Customer has been notified.`);
    }
  };
  
  // Handle adding a new event package
  const handleAddEventPackage = (newPackage) => {
    const packageWithId = {
      ...newPackage,
      id: `pkg-${Date.now()}`
    };
  ...
  const handleUpdateEventPackage = (updatedPackage) => {
  ...
  const handleDeleteEventPackage = (packageId) => {
    setEventPackages(prevPackages =>
